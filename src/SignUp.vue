...
const handlePhoneInput = (event: Event) => {
  ...
  // Force the input to only contain the validated value
  if (value !== validatedValue) {
    target.value = validatedValue
  }
  
  formData.phone = validatedValue
}
...
const handlePhoneKeypress = (event: KeyboardEvent) => {
  ...
  // Prevent input if already at max length (unless it's a control key)
  const target = event.target as HTMLInputElement
      ...
      <div v-if="currentStep === 1" class="min-h-screen relative gradient-bg">
        ...
        <div class="container mx-auto py-6 px-4 relative z-[2]">
          <div class="max-w-6xl mx-auto">
            <!-- Mobile Layout: Forms First, Content Below -->
